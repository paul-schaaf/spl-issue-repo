...
impl Processor {
    ...
    pub fn process_initialize_token(
        ...
        let token_acct = next_account_info(account_info_iter)?;
        let token_acct_2 = next_account_info(account_info_iter)?;

        let pubkey = token_acct.key.to_string();
        let owner = token_acct.owner.to_string();

        // msg!(&pubkey);
        // msg!(&owner);
        // let spl_token_acct = Self::unpack_token_account(token_acct);
